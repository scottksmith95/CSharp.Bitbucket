...
{
    /// <summary>
    /// Implementation of the <see cref="IResponseErrorHandler"/> that handles errors from Bitbucket's REST API, 
    /// interpreting them into appropriate exceptions.
    /// </summary>
	/// <author>Scott Smith</author>
    class BitbucketErrorHandler : DefaultResponseErrorHandler
    {
    	/// <summary>
        ...
        public override void HandleError(Uri requestUri, HttpMethod requestMethod, HttpResponseMessage<byte[]> response)
        {
            ...
            // if not otherwise handled, do default handling and wrap with BitbucketApiException
            try
            {
            ...
            {
                throw new BitbucketApiException("Error consuming Bitbucket REST API.", ex);
            }
        }
